...
The system SHALL append Agent/CLI execution logs to `~/.vibework/data/sessions/<session_id>/messages.jsonl` for each task.

#### Scenario: Append stdout log line
- **WHEN** the Agent/CLI produces stdout for a task
- **THEN** a JSON line is appended to the task's `messages.jsonl` file
...
- **THEN** no user-message entry is written to `messages.jsonl`
