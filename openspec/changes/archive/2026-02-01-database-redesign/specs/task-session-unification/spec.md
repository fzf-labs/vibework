...
### Requirement: Task is the canonical session identity
The system SHALL treat each task as the sole session for its execution context and SHALL persist `session_id` equal to `task.id`.

#### Scenario: Create task assigns session identity
- **WHEN** a task is created
- **THEN** the stored `session_id` equals the task's `id`
