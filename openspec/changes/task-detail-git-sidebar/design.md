...
**Goals:**
- 在任务详情页 Git 面板中用两个子 Tab 展示工作区变更与分支对比（基准分支 vs 当前分支）。
- 支持变更文件刷新与单文件暂存/取消暂存。
- 在无工作目录、非 Git 仓库或 Git 不可用时提供清晰的空状态/错误提示。

**Non-Goals:**
- 提交历史、分支管理与切换、提交/推送等复杂操作。
- 完整的 diff 浏览器或文件比较器。
...
- **分支对比基于 git diff --name-status**：使用 base branch 与当前分支进行 diff，展示文件级差异。
  - 备选：解析完整 diff 或只显示提交列表；未选因为本次仅需文件级提示。
