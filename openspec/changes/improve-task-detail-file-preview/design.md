...
任务详情页右侧目前只有预览容器，缺少可用的文件入口；文本/代码类文件常因缺少 content 而无法预览。现有 Electron 文件系统 API 已支持 readDir/readTextFile/stat，可用于补齐文件列表与内容加载。

## Goals / Non-Goals

**Goals:**
- 在右侧“文件预览”补齐文件列表（工作目录）并支持选择预览。
...
1) **文件列表结构**
- 在“文件预览”内加入列表面板，仅展示工作目录文件。
- 通过 `workingDir` 读取目录树，列表与预览同一视图分栏，使用可折叠/滚动的轻量文件树。
- 若存在分支信息，在列表顶部展示当前分支标签。
