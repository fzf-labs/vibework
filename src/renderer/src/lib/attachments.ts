 ...
 *
 * Structure: ~/.VibeWork/sessions/{sessionId}/attachments/{filename}
 */
...
export async function saveAttachmentToFile(
  ...
  if (!isElectron() || !attachment.data) {
    // In browser mode, return the original data (can't save to file system)
    return attachment.data;
  }

  try {
    const { fs } = await import('./electron-api');

    // Create attachments directory
    const attachmentsDir = `${sessionFolder}/attachments`;
    try {
      // Note: Electron fs doesn't have mkdir, we'll need to add it or use writeFile to create parent dirs
      // For now, we'll rely on writeFile creating parent directories
    } catch {
