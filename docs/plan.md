...
| 现状 | 优化后 |
|------|--------|
| 进入任务详情页自动开始执行 | 任务为 `todo` 状态时，需用户手动点击"开始任务"按钮 |

### 2. 开始按钮位置调整

| 现状 | 优化后 |
|------|--------|
| 开始按钮在 Agent CLI 输出框上 | 开始按钮放在任务级别（Task 层） |

**理由**: 开始按钮触发的是任务执行，应属于任务操作，而非 Agent CLI 操作

### 3. Agent CLI 输出自动滚动

| 现状 | 优化后 |
|------|--------|
| 输出内容停留在顶部 | 自动滚动到最新输出（底部） |
...
### 4. Agent CLI 完成状态的 UI 呈现

**问题**: Agent CLI 状态变为 `completed` 后，Work Node 联动变为 `in_review`，但页面无明显反馈

**优化方案**:

| 状态变化 | UI 呈现 |
|----------|---------|
| Agent CLI → `completed` | 显示"执行完成"提示 |
| Work Node → `in_review` | 显示"待确认"状态徽章 + 操作按钮 |

**操作按钮**:
- 「继续对话」→ Agent CLI 重新进入 `running`
- 「确认完成」→ Work Node 变为 `done`，进入下一节点